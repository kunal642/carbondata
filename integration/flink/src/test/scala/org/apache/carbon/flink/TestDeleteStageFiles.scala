...
class TestDeleteStageFiles extends QueryTest {
  ...
  test("Delete stage file success") {
    ...
    try {
      ...
      sql(s"DELETE FROM TABLE $tableName STAGE OPTIONS('location'='$dataPath', 'retain_hour'='0')")
      val dataLocation = new File(dataPath + CarbonCommonConstants.FILE_SEPARATOR + "default" + CarbonCommonConstants.FILE_SEPARATOR + tableName)
  ...
  test("All data files are referenced by stage, not delete any data files.") {
    ...
    try {
      ...
      sql(s"DELETE FROM TABLE $tableName STAGE OPTIONS('location'='$dataPath')")
      val dataLocation = new File(dataPath + CarbonCommonConstants.FILE_SEPARATOR + "default" + CarbonCommonConstants.FILE_SEPARATOR + tableName)
