...
import org.apache.spark.sql.execution.command.preaaggregate.PreAggregateUtil.getField
  ...
  private def transformAggregatePlan(logicalPlan: LogicalPlan): LogicalPlan = {
    logicalPlan transform {
      case a@Aggregate(_, aExp, _) =>
              ...
              case Average(attr: AttributeReference) =>
                Seq(Alias(attrExpression
                  .copy(aggregateFunction = Sum(attr.withName(attr.name)),
                    resultId = ExprId.apply(expressionId)),
                  attr.name)(),
                  Alias(attrExpression
                    .copy(aggregateFunction = Count(attr.withName(attr.name)),
                      resultId = ExprId.apply(expressionId + 1L)), attr.name)())
        ...
        a.copy(aggregateExpressions = newExpressions)
