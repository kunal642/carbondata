...
object SecondaryIndexUtil {
  ...
  .Map[String, java.lang.Long],
    ...
    try {
      rebuildSegmentProperty = CarbonProperties.getInstance().getProperty(
        ...
        CarbonCommonConstants.DEFAULT_CARBON_SI_SEGMENT_MERGE).toBoolean

    } catch {
      case _: Exception =>
        rebuildSegmentProperty = CarbonCommonConstants.DEFAULT_CARBON_SI_SEGMENT_MERGE
          .toBoolean
    }
    try {
      ...
      if (finalMergeStatus) {
        if (null != mergeStatus && mergeStatus.length != 0) {
          mergedSegments.asScala.map { seg =>
            ...
            SegmentFileStore.writeSegmentFile(indexCarbonTable,
              seg.getLoadName,
              file,
              indexCarbonTable.getCarbonTableIdentifier.getTableId, segment.getSegmentFileName)
            segment
          }
   ...
   */
  def prepareColumnCardinalityForIndexTable(carbonTable: CarbonTable,
      columnCardinalityForFactTable: Array[Int],
      indexTable: CarbonTable): Array[Int] = {
    var columnCardinalityForIndexTable: Array[Int] = null
    val factTableDimensions = carbonTable.getVisibleDimensions
    val indexTableDimensions = indexTable.getVisibleDimensions
    val factToIndexTableDimensionIndexMapping = new util.ArrayList[Integer](
    ...
    loop.breakable {
      for (indexTableDimension <- indexTableDimensions.asScala) {
        ...
        for (i <- 0 until factTableDimensions.size) {
          ...
          if (factTableDimensions.get(i).getColumnId == indexTableDimension.getColumnId) {
    ...
    if (factToIndexTableDimensionIndexMapping.isEmpty) {
      columnCardinalityForIndexTable = new Array(0)
    } else {
      columnCardinalityForIndexTable = new Array[Int](factToIndexTableDimensionIndexMapping.size)
      for (i <- 0 until factToIndexTableDimensionIndexMapping.size) {
        columnCardinalityForIndexTable(i) = columnCardinalityForFactTable(
          factToIndexTableDimensionIndexMapping.get(i))
      }
    }
    columnCardinalityForIndexTable
  }
}
