...
class TestCreateIndexTable extends QueryTest with BeforeAndAfterAll {
  ...
  override def beforeAll {
    ...
    sql("CREATE table carbon (empno int, empname String, " +
  ...
  test("test create index table with indextable col size > parent table key col size") {
    try {
      ...
      case ex: Exception =>
        assert(ex.getMessage.equalsIgnoreCase(
          "Secondary Index is not supported for measure column : attendance"))
    } finally{
...
//  }
  ...
  test("test if create secondary index creation throws stale state exception") {
    createAndInsertSecondaryIndex()
    sql("""alter table test1 change id id decimal(10,4)""")
    sql("""insert into test1 select 'xx',99.33 as a,'china'""")
    createAndInsertSecondaryIndex()
  }
